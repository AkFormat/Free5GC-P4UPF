...
// TODO: 需要看懂这个测试代码的过程，是否可独立运行？
func main() {
	var (
		server   = flag.String("-s", "127.0.0.8:8805", "server's addr/port") // 设置server地址
		nodeid   = flag.String("-n", "127.0.0.8", "client's node id")        //发送方node
		boottime = time.Now()
		seq      uint32
		err      error
		buf      = make([]byte, 1500)
	...
	sereq, err := message.NewSessionEstablishmentRequest(
		...
		seq,                           // Sequence Number
		0,                             // Message Priority
		ie.NewNodeID(*nodeid, "", ""), // 发送方ip
		ie.NewFSEID(1, net.ParseIP(*nodeid), nil), //Fseid = session + node id
		ie.NewCreatePDR(
			ie.NewPDRID(1),
			...
			ie.NewOuterHeaderRemoval(0 /* desc */, 0 /* ext */), // TODO: 需要了解这个是什么意思
			ie.NewFARID(1),
			ie.NewQERID(1),
		),
		ie.NewCreatePDR(
		...
		ie.NewCreateQER(
			...
			ie.NewMBR(2000000, 1000000), // 上下行带宽限制
			ie.NewQFI(1),
		),
