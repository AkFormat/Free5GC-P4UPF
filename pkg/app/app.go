...
func (u *UPF) Run() error {
	 ...
	 * context */
	go u.listenShutdownEvent() // 监听停止信号

	var err error
	u.driver, err = forwarder.NewDriver(&u.wg, u.cfg) // forwarder服务，数据面服务
	if err != nil {
		return err
	}
	
//	u.driver, err = forwarder.NewP4Driver(&u.wg, u.cfg)
//	if err != nil {
//		return err
//	}

	u.pfcpServer = pfcp.NewPfcpServer(u.cfg, u.driver) // 新建pfcp服务，控制面服务

	u.driver.HandleReport(u.pfcpServer)
	u.pfcpServer.Start(&u.wg) // 启动pfcp服务器

	logger.MainLog.Infoln("UPF started")

	// Wait for interrupt signal to gracefully shutdown
	sigCh := make(chan os.Signal, 1) // 信号通道
...
func (u *UPF) listenShutdownEvent() {
	...
	<-u.ctx.Done() // 阻塞，如果收到了停止信号，执行下面的代码
	u.pfcpServer.Stop()
	u.driver.Close()
}
