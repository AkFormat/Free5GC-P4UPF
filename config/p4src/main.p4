...
control Acl(
    ...
    action clone_to_cpu() {
        ...
        local_meta.copy_ingress_port = std_meta.ingress_port;
...
control PreQosPipe (inout parsed_headers_t    hdr,
    ...
    action set_source_iface(InterfaceType src_iface, Direction direction) { // 在本地变量中记录
    ...
    table source_iface_lookup {
        key = {
            hdr.ipv4.dst_addr: lpm; // 最长匹配，外层ipv4目的地址
    ...
    table pdrs {
        ...
        actions = {
            set_pdr_attributes; // 参数为：pdr_id_t, fseid_t, counter_index_t, far_id_t, needs_gtpu_decap(1)
            set_pdr_attributes_qos; // pdr_id_t，fseid_t，counter_index_t，far_id_t，needs_gtpu_decap（1）,needs_qfi_push（1），qfi（6）
        }
    }
    ...
    apply {
        ...
        if (hdr.packet_out.isValid()) {
            ...
            source_iface_lookup.apply(); // 检查数据包从那个网口而来, set_source_iface
            ...
            if (hdr.inner_ipv4.isValid()) { // 如果内部的ipv4是合法的
                ...
                hdr.outer_udp = hdr.udp; // 默认外4层使用UDP协议，因为GTP协议是基于UDP协议
            ...
            if (local_meta.direction == Direction.UPLINK) { // 如果流量是由UE传到UPF
            ...
            pdrs.apply(); // 应用pdr规则, set_pdr_attributes 或者 set_pdr_attributes_qos
