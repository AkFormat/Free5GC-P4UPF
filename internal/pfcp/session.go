...
func (s *PfcpServer) handleSessionEstablishmentRequest(
	...
	rnodeid, err := req.NodeID.NodeID() // CP的IP地址
	if err != nil {
		s.log.Errorln(err)
		return
	}
	...
	rnode, ok := s.rnodes[rnodeid] // 找到对应的rnode
	if !ok {
	...
	fseid, err := req.CPFSEID.FSEID() // CP下发的session ID
	if err != nil {
		s.log.Errorln(err)
		return
	}
	...
	// allocate a session
	sess := rnode.NewSess(fseid.SEID) // 新建session
