...
// 参考：https://nickvsnetworking.com/diameter-droplets-the-flow-description-avp/
// https://datatracker.ietf.org/doc/html/rfc6733#page-48
// IPFilterRule <- action s dir s proto s 'from' s src 'to' s dst
// action <- 'permit' / 'deny'，是否允许通过流量
// dir <- 'in' / 'out' ，"in" is from the terminal 上行流量, "out" is to the terminal 下行流量.
// proto <- 'ip' / digit，流量使用的协议（TCP 6  or UDP 17 ）
// src <- addr s ports?
// dst <- addr s ports?
...
func ParseFlowDesc(s string) (*FlowDesc, error) { //数据流的目标
	fd := new(FlowDesc)
