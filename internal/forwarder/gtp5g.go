...
func (g *Gtp5g) newPdi(i *ie.IE) (nl.AttrList, error) {
	...
	for _, x := range ies {
		switch x.Type {
		case ie.SourceInterface: // 对sourceInterface 不做判断？
		case ie.FTEID: // Full TEID
			v, err := x.FTEID()
			g.log.Infof("PDI() => FTEID: %v", v)
			if err != nil {
				break
			}
			attrs = append(attrs, nl.Attr{
				...
				Value: nl.AttrList{
					{
						...
						Value: nl.AttrU32(v.TEID), // v.TEID 为 int32类型
					},
					{
						Type:  gtp5gnl.F_TEID_GTPU_ADDR_IPV4,
						Value: nl.AttrBytes(v.IPv4Address), // 可能是gnb的Ipv4
					},
				},
			})
		case ie.NetworkInstance: // 对于NetworkInstance不用区分
		case ie.UEIPAddress: // UE IPv4的地址
			v, err := x.UEIPAddress()
			g.log.Infof("PDI() => UEIPAddress: %v", v)
			if err != nil {
				break
			}
			attrs = append(attrs, nl.Attr{
		...
		case ie.SDFFilter: // SDF过滤器, 在up4中是5元组
			v, err := g.newSdfFilter(x)
			g.log.Infof("PDI() => SDFFilter: %v", v)
			if err != nil {
				break
			}
			attrs = append(attrs, nl.Attr{
...
func (g *Gtp5g) CreatePDR(lSeid uint64, req *ie.IE) error {
	// lSeid = Session对象中的local session ID

	// 调用函数 InsertPDRtoSwitch
	var pdrid uint64
	var attrs []nl.Attr
	...
	for _, i := range ies {
		...
		case ie.PDRID:
			...
			pdrid = uint64(v)
		case ie.Precedence:
			v, err := i.Precedence() // PDR规则优先级，uint32类型
			if err != nil {
				break
			}
			attrs = append(attrs, nl.Attr{
		...
		case ie.OuterHeaderRemoval:
			v, err := i.OuterHeaderRemovalDescription() // action 参数，bool值
			if err != nil {
				break
			}
			attrs = append(attrs, nl.Attr{
				Type:  gtp5gnl.PDR_OUTER_HEADER_REMOVAL,
				Value: nl.AttrU8(v), // uint8类型
			})
			// ignore GTPUExternsionHeaderDeletion
		case ie.FARID: // action 参数
			v, err := i.FARID()
			if err != nil {
				break
			}
